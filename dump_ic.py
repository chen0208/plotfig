...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 21:58:25 2023
...
class Body:
    ...
    def __init__(self, npart,
        ...
        total_number_of_particles = np.sum(npart,dtype="int")
...
def ic_file(npart,total_number_of_particles,co,mass,velocities,u,fname):
    my_header=Header()
    my_body=Body(npart)
    ...
    my_body.id[:]=np.arange(0,total_number_of_particles)
    my_body.pos = co
    my_body.mass = mass
    my_body.vel = velocities
    my_body.u = u
    
    ic_file = open(fname,'wb')
    write_header(my_header,ic_file,format_output=1)
    write_body(my_body,ic_file,format_output=1)
